/*let openShopping = document.querySelector('.shopping');
let closeShopping = document.querySelector('.closeShopping');
let lista = document.querySelector('.lista');
let listCard = document.querySelector('.listCard');
let body = document.querySelector('body');
let total = document.querySelector('.total');
let quantity = document.querySelector('.quantity');

openShopping.addEventListener('click', ()=>{
    body.classList.add('active');
})
closeShopping.addEventListener('click', ()=>{
    body.classList.remove('active');
})

let products = [
    {
        id: 1,
        name: 'Cepillos de Dientes Eléctricos',
        IMAGENES: 'C.JPG',
        price: 120000
    },
    {
        id: 2,
        name: 'Cepillo Dental Oral-B',
        IMAGENES: 'C1.WEBP',
        price: 120000
    },
    {
        id: 3,
        name: 'Cepillo Dental Infantil',
        IMAGENES: 'C2.JPG',
        price: 220000
    },
    {
        id: 4,
        name: 'Pasta Dental Colgate Blanqueador',
        IMAGENES: 'C3.WEBP',
        price: 123000
    },
    {
        id: 5,
        name: 'Crema Colgate Carbon',
        IMAGENES: 'C4.WEBP',
        price: 320000
    },
    {
        id: 6,
        name: 'Pasta Dental Para Niños',
        IMAGENES: 'C5.JPEG',
        price: 120000
    },
    {
        id: 7,
        name: 'clorex enjuague bucal clorhexidina',
        IMAGENES: 'E1.WEBP',
        price: 12000
    },
    {
        id: 8,
        name: 'Listerine enjuague bucal cuidado total',
        IMAGENES: 'E2.WEBP',
        price: 12000
    },
    {
        id: 9,
        name: 'Pasta Dental Para Niños',
        IMAGENES: 'E3.WEBP',
        price: 12000
    }
    
    
];
let listCards  = [];
function initApp(){
    products.forEach((value, key) =>{
        let newDiv = document.createElement('div');
        newDiv.classList.add('item');
        newDiv.innerHTML = `
            <img src="IMAGENES/${value.IMAGENES}">
            <div class="title">${value.name}</div>
            <div class="price">${value.price.toLocaleString()}</div>
            <button onclick="addToCard(${key})">Agregar al Carrito</button>`;
        lista.appendChild(newDiv);
    })
}
initApp();
function addToCard(key){
    if(listCards[key] == null){
        // copiar el formulario del producto lista a la tarjeta lista
        listCards[key] = JSON.parse(JSON.stringify(products[key]));
        listCards[key].quantity = 1;
    }
    reloadCard();
}
function reloadCard(){
    listCard.innerHTML = '';
    let count = 0;
    let totalPrice = 0;
    listCards.forEach((value, key) => {
        totalPrice = totalPrice + value.price;
        count = count + value.quantity;
        if (value != null) {
            let newDiv = document.createElement('li');
            newDiv.innerHTML = `
                <div><img src="IMAGENES/${value.IMAGENES}"/></div>
                <div>${value.name}</div>
                <div>${value.price.toLocaleString()}</div>
                <div>
                    <button onclick="changeQuantity(${key}, ${value.quantity - 1})">-</button>
                    <div class="count">${value.quantity}</div>
                    <button onclick="changeQuantity(${key}, ${value.quantity + 1})">+</button>
                    <button onclick="removeFromCard(${key})">Remove from Card</button>
                </div>`;
            listCard.appendChild(newDiv);
        }
    });
    total.innerText = totalPrice.toLocaleString();
    quantity.innerText = count;
}



function changeQuantity(key, quantity){
    if(quantity == 0){
        delete listCards[key];
    }else{
        listCards[key].quantity = quantity;
        listCards[key].price = quantity * products[key].price;
    }
    reloadCard();
}

function removeFromCard(key) {
    delete listCards[key];
    reloadCard();
}*/






/*
let openShopping = document.querySelector('.shopping');
let closeShopping = document.querySelector('.closeShopping');
let lista = document.querySelector('.lista');
let listCard = document.querySelector('.listCard');
let body = document.querySelector('body');
let total = document.querySelector('.total');
let quantity = document.querySelector('.quantity');

openShopping.addEventListener('click', () => {
    body.classList.add('active');
});
closeShopping.addEventListener('click', () => {
    body.classList.remove('active');
});

let products = [
    {
        id: 1,
        name: 'Cepillos de Dientes Eléctricos',
        IMAGENES: 'C.JPG',
        price: 120000
    },
    {
        id: 2,
        name: 'Cepillo Dental Oral-B',
        IMAGENES: 'C1.WEBP',
        price: 120000
    },
    {
        id: 3,
        name: 'Cepillo Dental Infantil',
        IMAGENES: 'C2.JPG',
        price: 220000
    },
    {
        id: 4,
        name: 'Pasta Dental Colgate Blanqueador',
        IMAGENES: 'C3.WEBP',
        price: 123000
    },
    {
        id: 5,
        name: 'Crema Colgate Carbon',
        IMAGENES: 'C4.WEBP',
        price: 320000
    },
    {
        id: 6,
        name: 'Pasta Dental Para Niños',
        IMAGENES: 'C5.JPEG',
        price: 120000
    },
    {
        id: 7,
        name: 'clorex enjuague bucal clorhexidina',
        IMAGENES: 'E1.WEBP',
        price: 12000
    },
    {
        id: 8,
        name: 'Listerine enjuague bucal cuidado total',
        IMAGENES: 'E2.WEBP',
        price: 12000
    },
    {
        id: 9,
        name: 'Pasta Dental Para Niños',
        IMAGENES: 'E3.WEBP',
        price: 12000
    }
    
    // Resto de los productos...
];

let listCards = [];

function initApp() {
    products.forEach((value, key) => {
        let newDiv = document.createElement('div');
        newDiv.classList.add('item');
        newDiv.innerHTML = `
            <img src="IMAGENES/${value.IMAGENES}" onclick="enlargeImage(this)">
            <div class="title">${value.name}</div>
            <div class="price">${value.price.toLocaleString()}</div>
            <button onclick="addToCard(${key})">Agregar al Carrito</button>`;
        lista.appendChild(newDiv);
    });
}

initApp();

function addToCard(key) {
    if (listCards[key] == null) {
        listCards[key] = JSON.parse(JSON.stringify(products[key]));
        listCards[key].quantity = 1;
    }
    reloadCard();
}

function reloadCard() {
    listCard.innerHTML = '';
    let count = 0;
    let totalPrice = 0;
    listCards.forEach((value, key) => {
        totalPrice = totalPrice + value.price;
        count = count + value.quantity;
        if (value != null) {
            let newDiv = document.createElement('li');
            newDiv.innerHTML = `
                <div><img src="IMAGENES/${value.IMAGENES}"/></div>
                <div>${value.name}</div>
                <div>${value.price.toLocaleString()}</div>
                <div>
                    <button onclick="changeQuantity(${key}, ${value.quantity - 1})">-</button>
                    <div class="count">${value.quantity}</div>
                    <button onclick="changeQuantity(${key}, ${value.quantity + 1})">+</button>
                    <button onclick="removeFromCard(${key})">Eliminar del Carrito</button>
                </div>`;
            listCard.appendChild(newDiv);
        }
    });
    total.innerText = totalPrice.toLocaleString();
    quantity.innerText = count;
}

function changeQuantity(key, quantity) {
    if (quantity == 0) {
        delete listCards[key];
    } else {
        listCards[key].quantity = quantity;
        listCards[key].price = quantity * products[key].price;
    }
    reloadCard();
}

function removeFromCard(key) {
    delete listCards[key];
    reloadCard();
}

function enlargeImage(image) {
    image.classList.toggle('enlarged');
}


//EVENTO DE COLOR
$('onclick').on("Click", botao)

function botao(){
    $('li').css({'color':'blue'})
}*/









let openShopping = document.querySelector('.shopping');
let closeShopping = document.querySelector('.closeShopping');
let lista = document.querySelector('.lista');
let listCard = document.querySelector('.listCard');
let body = document.querySelector('body');
let total = document.querySelector('.total');
let quantity = document.querySelector('.quantity');

openShopping.addEventListener('click', () => {
    body.classList.add('active');
});
closeShopping.addEventListener('click', () => {
    body.classList.remove('active');
});

let products = [
    {
        id: 1,
        name: 'Cepillos de Dientes Eléctricos',
        IMAGENES: 'CE1.JPG',
        price: 120000
    },
    {
        id: 2,
        name: 'Cepillo Dental Oral-B',
        IMAGENES: 'CE2.JPG',
        price: 120000
    },
    {
        id: 3,
        name: 'Cepillo Dental Infantil',
        IMAGENES: 'CE3.JPG',
        price: 220000
    },
    {
        id: 4,
        name: 'Pasta Dental Colgate Blanqueador',
        IMAGENES: 'CE4.JPG',
        price: 123000
    },
    {
        id: 5,
        name: 'Crema Colgate Carbon',
        IMAGENES: 'CE5.JPG',
        price: 320000
    },
    {
        id: 6,
        name: 'Pasta Dental Para Niños',
        IMAGENES: 'CE6.JPG',
        price: 120000
    },
    {
        id: 7,
        name: 'clorex enjuague bucal clorhexidina',
        IMAGENES: 'CE7.PNG',
        price: 12000
    },
    {
        id: 8,
        name: 'Listerine enjuague bucal cuidado total',
        IMAGENES: 'CE8.PNG',
        price: 12000
    },
    {
        id: 9,
        name: 'Pasta Dental Para Niños',
        IMAGENES: 'CE9.JPG',
        price: 12000
    }
    
    // Resto de los productos...
];

let listCards = [];

function initApp() {
    products.forEach((value, key) => {
        let newDiv = document.createElement('div');
        newDiv.classList.add('item');
        newDiv.innerHTML = `
            <img src="IMAGENES/${value.IMAGENES}" onclick="enlargeImage(this)">
            <div class="title">${value.name}</div>
            <div class="price">${value.price.toLocaleString()}</div>
            <button onclick="addToCard(${key})">Agregar al Carrito</button>`;
        lista.appendChild(newDiv);
    });
}

initApp();

function addToCard(key) {
    if (listCards[key] == null) {
        listCards[key] = JSON.parse(JSON.stringify(products[key]));
        listCards[key].quantity = 1;
    }
    reloadCard();
}

function reloadCard() {
    listCard.innerHTML = '';
    let count = 0;
    let totalPrice = 0;
    listCards.forEach((value, key) => {
        totalPrice = totalPrice + value.price;
        count = count + value.quantity;
        if (value != null) {
            let newDiv = document.createElement('li');
            newDiv.innerHTML = `
                <div><img src="IMAGENES/${value.IMAGENES}"/></div>
                <div>${value.name}</div>
                <div>${value.price.toLocaleString()}</div>
                <div>
                    <button onclick="changeQuantity(${key}, ${value.quantity - 1})">-</button>
                    <div class="count">${value.quantity}</div>
                    <button onclick="changeQuantity(${key}, ${value.quantity + 1})">+</button>
                    <button onclick="removeFromCard(${key})">Eliminar del Carrito</button>
                </div>`;
            listCard.appendChild(newDiv);
        }
    });
    total.innerText = totalPrice.toLocaleString();
    quantity.innerText = count;
}

function changeQuantity(key, quantity) {
    if (quantity == 0) {
        delete listCards[key];
    } else {
        listCards[key].quantity = quantity;
        listCards[key].price = quantity * products[key].price;
    }
    reloadCard();
}

function removeFromCard(key) {
    delete listCards[key];
    reloadCard();
}

function enlargeImage(image) {
    image.classList.toggle('enlarged');
}

